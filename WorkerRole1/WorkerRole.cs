...
namespace WorkerRole1
{
    public class WorkerRole : RoleEntryPoint
    {
        ...
        private void getstatus()
        {
            ...
            CloudTable table = getTable();
            TableQuery<Node> query = new TableQuery<Node>().Where(TableQuery.GenerateFilterCondition("PartitionKey", QueryComparisons.Equal, "asgn"));
            int urlscount = crawler.urlscount;
            int queuesize = crawler.queuesize;
            int tablesize = 0;
            string temp = "";
            string secondary = "";
            foreach (Node entity in table.ExecuteQuery(query))
            {
                tablesize++;
            }
            if (crawler.inserturls != null)
            {
                ...
                asgn3.dashboard replace = new dashboard(urlscount, temp, queuesize, tablesize, secondary);
                TableOperation insertOperation = TableOperation.InsertOrReplace(replace);
                table.ExecuteAsync(insertOperation);
        }
